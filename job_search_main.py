...
def select_sentence():
    ...
    "3": "Communicate with internal and external teams to effectively create and consume API endpoints.",
...
def create_resume(local_words, company, job_title, location, url):
    ...
    records = (
        ('', ['Familiar Languages', 'Python', 'SQL', 'Javascript', 'PHP', 'CSS', 'HTML']),
    ...
    while saving:
        try:
            document.save(f"{job_title} at {company}.docx".replace('  ', ' '))
            saving = False
        except:
